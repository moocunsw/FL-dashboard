...
ui <- fluidPage(
  headerPanel("Comments Overview"),
  fluidRow(
    h4("The following table shows the basic statistics about the comments given by learners such as total number of comments and likes, unique number of authors, median/average word count per comment or per person, and average/maximum number of reply per comment."),
    DT::dataTableOutput('CommentsOverviewTable',width = 600)
...
server <- function(input, output, session) {
  ...
  commentsOverview <- reactive({
    ...
    if (!is.null(query[['course']]))
    {
      ...
      rs = dbSendQuery(mydb, "select stat_type as `Metric`, value as `Value` from vis_CommentsOverviewTable")
      data = fetch(rs, n=-1)
  ...
  output$CommentsOverviewTable <- renderDataTable(
    {
