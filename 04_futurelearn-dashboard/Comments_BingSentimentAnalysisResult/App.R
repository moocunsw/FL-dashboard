...
ui <- fluidPage(
  fluidRow(showOutput("BingSentimentAnalysisResult", "highcharts"))
  ) 
...
server <- function(input, output, session) {
  ...
  doSentimentAnalysis <- reactive({
    ...
    if (!is.null(query[['course']]))
    {
      ...
      rs = dbSendQuery(mydb, "select * from vis_BingSentimentAnalysisResult")
      fr = fetch(rs, n=-1)
      dbClearResult(rs)
      dbDisconnect(mydb)
  ...
  output$BingSentimentAnalysisResult <- renderChart(
    {
      query <- parseQueryString(session$clientData$url_search)
      if (!is.null(query[['course']]))
      {
        ...
        withProgress(message = 'Reading Data...', value = 0, {
          ...
          h1 <- hPlot(
            ...
            subtitle ='<a href="https://www.cs.uic.edu/~liub/FBS/sentiment-analysis.html">Bing Method</a> has been used to find the sentiment score of comments. ')
        ...
        h1$addParams(dom = 'BingSentimentAnalysisResult')
        h1$chart(height=700)
        return(h1)
      }
    })
